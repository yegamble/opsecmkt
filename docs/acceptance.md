...
- [x] A conflicted credited deposit is flagged to moderators without reverting the order, and its unsent payouts are held. A credited deposit back below the threshold holds the payout until it re-confirms (`TestWatcherFlagsConflictAfterPaidWithoutReverting`, `TestPayoutHeldWhileCreditedDepositReconfirms`).

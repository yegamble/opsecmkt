...
| A-24 | Payment-review flags lead nowhere: staff notified by `flagOrder` get 404 on the order (`disputeReviewer` allows only disputed/resolved) and /moderator lists only disputes (PM-1, MOD-1, DA-4, QA3-1 — reproduced by QA) | payments/staff | Go DB probe (QA, scratch) | fixed (war-room/integration; fix/a24-flag-review f432182 + e1bfcd9) |
| A-25 | Admin payouts panel `ORDER BY p.id DESC LIMIT 50` (`payments_actions.go:279`): an older failed/held/stuck payout and its only recovery form drop off the page (MOD-3) | payments/staff | code read | fixed (war-room/integration; fix/a25-payout-attention 675ffc6 + e1bfcd9) |
...
| A-47 | Adding a second factor needs only the session: PGP key change + `/pgp/2fa` enable (confirmed only for disable, `pgp_actions.go:257`) and `/totp/enroll`+`/activate` on a PGP-sign-in account; `/challenge` accepts any enrolled factor, so an attacker with a session (and the password) adds a factor that survives session revocation (HUNT-D-1, bug hunt #1) | identity | Go DB repro, verified (opsec-bug-verifier) | open |
| A-48 | A cross-site link to the market signs the user out: a cookieless (SameSite=Strict withheld) request gets a fresh anonymous `session` cookie that overwrites the real one (`app.go:196-214`) (HUNT-D-2, bug hunt #1) | identity/ui | Go DB + real Chromium & Firefox, verified | open |
...
- **A-52 (P0; raised from the verifier's REQUIRED by the chair: a payout including a deposit the ledger already marks conflicted breaks the hold invariant).** ACCEPTANCE (verifier's safe shape): `enqueuePayout` returns early when a payout row exists; otherwise `SELECT id,amount FROM payments WHERE <settled> FOR UPDATE`, sums in Go, inserts the payout, then `UPDATE payments SET credited=true WHERE id=ANY($ids)` — the payout amount equals exactly the credited rows; the hold check sees every credited row. TEST: the hunt's `TestHuntRefundSilentlyDropsDepositConfirmedDuringEnqueue` and the verifier's `TestVerifyE2NaturalRace`/`TestVerifyE2ReverseRace` (artifacts/bug-hunt/699ea45/verify/verify2_payments_test.go.txt) adapted into `payments_watcher_gaps_test.go`, passing repeatedly under `-race`.
...
- **A-47 (MODIFY, bug hunt #1)**. DISSENT: the verifier would also require a PGP proof before enrolling TOTP on a PGP-only account; rejected for now because the existing rule already accepts the password alone to turn PGP sign-in *off* on such an account, so password-only enabling is no weaker (revisit with A-32). ACCEPTANCE: when the account has any second factor enrolled, enabling another (`/pgp/2fa` enable, `/totp/activate`) and changing the saved PGP key require `confirmedTx(c, true, …)` — current password plus a current TOTP code or unused recovery code when TOTP is enrolled; forms render `confirm-fields`; an account with no factor keeps today's flow; audit rows say how it was confirmed. TEST: the hunt's `TestHuntStolenSessionAddsPGPFactorBypassingTOTP` and `TestHuntStolenSessionEnrollsTOTPBypassingPGP` moved into `auth_gaps_test.go` and passing; existing TOTP/PGP Playwright specs updated.
